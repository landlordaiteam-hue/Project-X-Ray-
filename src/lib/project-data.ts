...
export async function updateProject(
  ...
  if (updates.code && projects.some((project, idx) => idx !== index && project.code.toLowerCase() === updates.code?.toLowerCase())) {
    throw new Error('Project code already exists');
  }

  const next = { ...projects[index], ...updates };
  projects = projects.map((project, idx) => (idx === index ? next : project));
  return next;
}
...
export async function addProjectMember(projectId: string, member: Pick<ProjectMember, 'name' | 'role'>): Promise<ProjectMember | null> {
  ...
  if (project.members.some((item) => item.id === nextMember.id || item.name.toLowerCase() === member.name.toLowerCase())) {
